function [pSkelMatch] = combine_skel_and_orig(pSkel,pOrig)
...
for a = 1:length(pSkel)
    ...
    if(isempty(anyIn) == 0)
        ...
        pSkelMatch(a).pOrig = pOrig(anyIn);
        pSkelMatch(a).Area = pOrig(anyIn).Area;
        if(length(anyIn) == 1)
